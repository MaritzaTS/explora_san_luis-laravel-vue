<template>
  <div>
      ...
      <div class="row g-0">
        ...
        <div class="col-md-9 d-flex align-items-center">
          <div class="card-body p-3">
            <h5 class="fw-bold mb-1">Gestión de Agencias Turísticas</h5>
    ...
    <div class="row g-4 mb-4">
      <div class="col-md-4">
        <div class="card border-0 shadow-sm p-4 rounded-4 bg-white border-start border-success border-4">
          <h5 class="fw-bold mb-3 text-success">Total Activos</h5>
      ...
      <div class="col-md-4">
        <div class="card border-0 shadow-sm p-4 rounded-4 bg-white border-start border-danger border-4">
          <h5 class="fw-bold mb-3 text-danger">Total Inactivos</h5>
      ...
      <div class="col-md-4">
        <div class="card border-0 shadow-sm p-4 rounded-4 bg-white border-start border-dark border-4">
          <h5 class="fw-bold mb-3 text-muted">Total Registros</h5>
    ...
    <!-- ===== MODAL AGREGAR ===== -->
    <div class="modal fade" id="modalAgregarAgencia" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow-lg">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold d-flex align-items-center">
              <i class="bi bi-pencil-square fs-4 me-2"></i> Agregar Agencia Turística
            </h5>
          ...
          <div class="modal-body p-4">
            <form @submit.prevent="guardarNueva">
              <div class="row g-4">
                ...
                <div class="col-md-6">
                  <label class="form-label fw-bold mb-1">
                    <i class="bi bi-image me-1"></i> Imagen Principal (Banner) *
                  </label>
                  <small class="text-muted d-block mb-2">Se mostrará en la cabecera de la entidad.</small>
                  <input type="file" ref="inputBanner" accept="image/*"
                    class="form-control border-0 border-bottom border-dark rounded-0 px-0 shadow-none" required />
                </div>
                <div class="col-md-6">
                  <label class="form-label fw-bold mb-1">
                    <i class="bi bi-person-badge me-1"></i> Logo de la Agencia *
                  </label>
                  <small class="text-muted d-block mb-2">Imagen cuadrada o logo distintivo.</small>
                  <input type="file" ref="inputLogo" accept="image/*"
                    class="form-control border-0 border-bottom border-dark rounded-0 px-0 shadow-none" required />
                </div>
              </div>
              ...
              <div class="d-flex justify-content-center gap-3 mt-5">
                <button type="button"
                  class="btn btn-outline-dark border-secondary-subtle px-4 py-2 fw-bold rounded-3 shadow-none"
    ...
    <!-- ===== MODAL VER DETALLE ===== -->
    <div class="modal fade" id="modalDetalleAgencia" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow-lg">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">Detalle de la Agencia</h5>
            <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
          </div>
            ...
            <div class="row g-4 align-items-start">
              ...
              <div class="col-md-8">
                ...
                <p class="text-primary fw-semibold mb-3 small">
                  <i class="bi bi-tag-fill me-1"></i> Agencia Verificada
                </p>
                <div class="row g-3">
                  <div class="col-md-6">
        ...
        <div class="modal-content border-0 rounded-4 shadow-lg">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">
              <i class="bi bi-pencil-fill me-2"></i> Editar Agencia Turística
            </h5>
            <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
          </div>
...
const ID_TIPO = 5  // Agencias Turísticas
...
const totalGeneral  = computed(() => entidades.value.length)
const totalActivos  = computed(() => entidades.value.filter(e => e.estado == 1).length)
const totalInactivos = computed(() => entidades.value.filter(e => e.estado == 0).length)
...
async function cargarEntidades() {
  try {
    ...
    mostrarAlerta('danger', '¡Error!', 'No se pudieron cargar las agencias.')
  } finally {
...
function verDetalle(item) {
  detalle.value = item
  new bootstrap.Modal(document.getElementById('modalDetalleAgencia')).show()
}
...
function abrirModalAgregar() {
  ...
  new bootstrap.Modal(document.getElementById('modalAgregarAgencia')).show()
}
...
async function guardarNueva() {
  ...
  fd.append('id_tipo', ID_TIPO)
  if (inputBanner.value?.files[0]) fd.append('url_imagen_principal', inputBanner.value.files[0])
  if (inputLogo.value?.files[0])   fd.append('url_imagen_tipo_logo', inputLogo.value.files[0])
  await axios.post('/api/admin/entidades', fd)
  bootstrap.Modal.getInstance(document.getElementById('modalAgregarAgencia')).hide()
  mostrarAlerta('success', '¡Guardado!', 'Agencia turística agregada correctamente.')
  await cargarEntidades()
}

function abrirModalEditar(item) {
  formEditar.value = { ...item, logoPreview: item.url_imagen_tipo_logo }
  new bootstrap.Modal(document.getElementById('modalEditarAgencia')).show()
}
...
async function guardarEdicion() {
  ...
  bootstrap.Modal.getInstance(document.getElementById('modalEditarAgencia')).hide()
  mostrarAlerta('success', '¡Actualizado!', 'Agencia turística editada correctamente.')
  await cargarEntidades()
}
